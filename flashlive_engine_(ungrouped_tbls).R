fun_tournament_list_u <- function(loc = locale$locale[1],
                                  sport = 1) {
  ...
  dbWriteTable(con, 
               c("sport_data", str_glue("{sport_name}_u_tournament_list")),
               overwrite = T,
               row.names = F,
...
fun_events_list_u <- function(loc = locale$locale[1],
                              tz = 3,
                              days,
                              sport = 1) {
  ...
  if (days < 0) {
    ...
    dt_name <- str_glue("{sport_name}_u_event_list_{abs(days)}_days_ago")
    
  } else if (days == 0) {
    
    dt_name <- str_glue("{sport_name}_u_event_list_today")
    
  } else {
    
    dt_name <- str_glue("{sport_name}_u_event_list_{abs(days)}_days_after")
...
fun_event_odds_u <- function(loc = locale$locale[1],
                             ev_id,
                             sport = 1) {
  ...
  dbWriteTable(con,
               c("sport_data", str_glue("{sport_name}_odds_u")),
               overwrite = F,
               append = T,
               row.names = F,
               value = DT)
...
fun_event_statistics_u <- function(loc = locale$locale[1],
                                   ev_id,
                                   sport = 1) {
  ...
  DT <- data.table(tibble(stats = event_statistics) |>
                     unnest_wider(stats) |>
                     unnest(DATA) |>
                     unnest_wider(DATA) |>
                     unnest(GROUPS) |>
  ...
  dbWriteTable(con, c("sport_data", str_glue("{sport_name}_statistics_u")),
               overwrite = F,
               append = T,
               row.names = F,
               value = DT)
...
fun_event_hth_u <- function(loc = locale$locale[1],
                            ev_id,
                            sport = 1) {
  ...
  dbWriteTable(con, c("sport_data", str_glue("{sport_name}_hth_u")),
               overwrite = T,
               row.names = F,
               value = DT)
